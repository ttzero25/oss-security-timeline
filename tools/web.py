...
def reports_page(reports: list[dict], selected_key: str | None = None, document: str = "ghsa") -> str:
    ...
    for item in reports:
        ...
        cards += f'''<article class="report-card"><div class="report-card-head"><span class="report-state {state_class}">{state}</span><small>{esc(str(item.get("validated_at") or "검증 기록 없음")[:19])}</small></div><h2>{esc(item["title"])}</h2><dl><div><dt>타겟</dt><dd>{esc(item["repo"])}</dd></div><div><dt>분석 커밋</dt><dd><code>{esc(item["commit"][:12])}</code></dd></div><div><dt>후보 ID</dt><dd>{esc(item["finding_id"])}</dd></div><div><dt>유형</dt><dd>{esc(item["kind"] or "미기재")}</dd></div><div><dt>위치</dt><dd>{esc(item["path"])}:{esc(item["sink_line"])}</dd></div><div><dt>생성기</dt><dd>{esc(item.get("generator") or "수동/이전 형식")}</dd></div><div><dt>검증 범위</dt><dd>{esc(scope_labels.get(item.get("proof_scope"), item.get("proof_scope") or "이전 형식 · 범위 미기재"))}</dd></div><div><dt>격리 방식</dt><dd>{esc(item.get("verification_mode") or "미실행")}</dd></div><div><dt>제출 참조</dt><dd>{esc(submission.get("reference") or "기록 없음")}</dd></div></dl>{links}</article>'''
    viewer = ""
    if selected:
