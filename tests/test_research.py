...
'''
    ...
    def test_python_path_poc_keeps_traversal_fixture_inside_scratch(self):
        with tempfile.TemporaryDirectory() as temp:
            folder = Path(temp)
            repo = folder / "repo"
            repo.mkdir()
            (repo / "reader.py").write_text('''def read_file(path):\n    return open(path, "r").read()\n''', encoding="utf-8")
            subprocess.run(["git", "init", "-q", str(repo)], check=True)
            subprocess.run(["git", "-C", str(repo), "add", "reader.py"], check=True)
            subprocess.run(["git", "-C", str(repo), "-c", "user.name=Fixture", "-c", "user.email=fixture@example.test", "commit", "-qm", "fixture"], check=True)
            audit_file = audit(repo, "fixture/python-path-poc", folder / "research")
            audit_data = json.loads(audit_file.read_text())
            finding = next(item for item in SemanticAnalysisAgent().run(audit_data["hypotheses"], audit_data["profile"]) if item["kind"] == "path_traversal")
            self.assertTrue(finding["auto_reproduction_supported"])
            manifest_file = LimitedPocAgent().run(audit_file, finding, BuildEnvironmentAgent().run(repo, finding))
            manifest = json.loads(manifest_file.read_text())
            self.assertEqual(manifest["filesystem_policy"], "scratch_read_only_fixture")
            evidence = json.loads(PocValidatorAgent().run(manifest_file).read_text())
            self.assertEqual(evidence["mechanical_result"], "contrast_matched")
            self.assertFalse(evidence["control_observable"])
