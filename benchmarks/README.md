...
The bundled synthetic cases include fixed-command allowlists, imported Python class-method paths, JavaScript cross-module calls and scope isolation, Go local-package calls and shell-free argv execution, and regression cases for SSRF, SQL/template injection, and filesystem paths. The `real/` directory adds focused source excerpts from public vulnerable/fixed refs. Every historical case must record an advisory, repository, immutable ref, source path, snapshot type, and vulnerable/fixed `pair_id`; it is not accepted as anonymous synthetic code.
